...
share8_covid1 <- data_read(
  "./Daten/Share8-Corona/sharew8ca_rel8-0-0_ca.zip",
  convert_factors = FALSE
)
share8_covid1_at <- data_read(
  "./Daten/Share8-Corona/sharew8ca_rel8-0-0_ca_at.zip",
  convert_factors = FALSE
)

# copy country variable
share8_covid1$countries <- to_factor(share8_covid1$country)
share8_covid1_at$countries <- to_factor(share8_covid1_at$country)
# add data from Austria
share8_covid1 <- data_merge(share8_covid1, share8_covid1_at, join = "bind")
share8_covid2 <- data_read(
  "./Daten/Share8-Corona/sharew8ca_rel8-0-0_cv_r.zip",
  convert_factors = FALSE
)
...
share8_weights <- data_read(
  "./Daten/Share8-Corona/sharew8ca_rel8-0-0_gv_weights_ca.zip",
  convert_factors = FALSE
)

share8_weights <- data_rename(
  share8_weights,
...
share9_covid1 <- data_read(
  "./Daten/Share9-Corona/sharew9ca_rel8-0-0_ca.zip",
  convert_factors = FALSE
)
share9_covid2 <- data_read(
  "./Daten/Share9-Corona/sharew9ca_rel8-0-0_cv_r.zip",
  convert_factors = FALSE
)
...
share9_weights <- data_read(
  "./Daten/Share9-Corona/sharew9ca_rel8-0-0_gv_weights_ca.zip",
  convert_factors = FALSE
)

share9_weights <- data_rename(
  share9_weights,
...
not_needed <- startsWith(colnames(share_8_9), "rel_") | endsWith(colnames(share_8_9), ".y")
...
share8_is <- data_read(
  "./Daten/Share8/sharew8_rel8-0-0_gv_isced.zip",
  convert_factors = FALSE
)
share8_is$isced1997_r <- convert_to_na(share8_is$isced1997_r, na = 95)
share8_is$education2 <- recode_values(
  share8_is$isced1997_r,
  recode = list(`0` = 0:2, `1` = 3:4, `2` = 5:6) ,
  default = NA
)
share8_is$education2 <- to_factor(
  sjlabelled::set_labels(share8_is$education2, labels = c("low", "mid", "high"))
)
...
share_8_9 <- sjlabelled::drop_labels(share_8_9)
share <- share_8_9
...
# recodings -------------------
source("./Analysen/final/data_preparation/02_recodes.R")
