...
oxford_reduced <- oxford_reduced |>
  dplyr::group_by(CountryName) |>
  dplyr::mutate(
    avg_str_index = mean(daily_index, na.rm = TRUE),
...
oxford_for_merge <- oxford_reduced |>
  ...
  dplyr::ungroup() |>
  dplyr::select(-c(2:39, 42:47, 56:65), -ends_with("ForDisplay"), -ends_with("Index", FALSE)) |>
...
oxford_reduced <- oxford |>
  dplyr::group_by(CountryName) |>
  dplyr::filter(Date > Fieldwork8 & Date <= Fieldwork) |>
  dplyr::ungroup()
...
oxford_for_merge <- oxford_reduced |>
  ...
  dplyr::filter(Date == max(Date, na.rm = TRUE)) |>
  dplyr::ungroup() |>
  dplyr::select(-c(2:39, 42:47, 56:65), -ends_with("ForDisplay"), -ends_with("Index", FALSE), -Fieldwork8) |>
  sjmisc::rename_columns(CountryName = "country")
...
# final data ------------------------------
oxford_for_merge <- dplyr::bind_rows(oxford_for_merge_wave8, oxford_for_merge_wave9)
...
share$covid_regime_si3 <- sjlabelled::set_label(share$covid_regime_si3, label = "Stringency Index 3 Groups")
...
share$global_covid_regime_si3 <- sjlabelled::set_label(share$global_covid_regime_si3, label = "Global Stringency Index 3 Groups")
...
share$covid_regime_gr3 <- sjlabelled::set_label(share$covid_regime_gr3, label = "Government Response Index 3 Groups")
...
share$global_covid_regime_gr3 <- sjlabelled::set_label(share$global_covid_regime_gr3, label = "Global Government Response Index 3 Groups")
...
share$covid_regime_ch3 <- sjlabelled::set_label(share$covid_regime_ch3, label = "Containment and Health Index 3 Groups")
...
share$global_covid_regime_ch3 <- sjlabelled::set_label(share$global_covid_regime_ch3, label = "Global Containment and Health Index 3 Groups")
