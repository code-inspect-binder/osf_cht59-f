...
share$cah121_1 <- recode_values(share$cah121_1, recode = list(`1` = 3, `2` = 1, `3` = 2), preserve_na = TRUE)
share$cah121_1 <- sjlabelled::set_labels(share$cah121_1, labels = sjlabelled::get_labels(share$cah021_))

share$camh113_1 <- recode_values(share$camh113_1, recode = list(`1` = 3, `2` = 1, `3` = 2), preserve_na = TRUE)
share$camh113_1 <- sjlabelled::set_labels(share$camh113_1, labels = sjlabelled::get_labels(share$camh802_))

share$camh118_1 <- recode_values(share$camh118_1, recode = list(`1` = 3, `2` = 1, `3` = 2), preserve_na = TRUE)
share$camh118_1 <- sjlabelled::set_labels(share$camh118_1, labels = sjlabelled::get_labels(share$camh807_))

share$cah102_ <- recode_values(share$cah102_, recode = list(`1` = 1, `2` = 3, `3` = 2), preserve_na = TRUE)
share$cah102_ <- sjlabelled::set_labels(share$cah102_, labels = sjlabelled::get_labels(share$cah002_))

share$camh148_ <- recode_values(share$camh148_, recode = list(`1` = 3, `2` = 1, `3` = 2), preserve_na = TRUE)
share$camh148_ <- sjlabelled::set_labels(share$camh148_, labels = sjlabelled::get_labels(share$camh837_))
...
merge_vars <- function(x, vars) {
  for (i in seq_along(vars)) {
    ...
    if (all(labs1 == labs2)) {
      ...
      x[[vars[[i]][2]]] <- NULL
      if (!is.null(lab1) && !is.null(lab2) && endsWith(lab1, "outbreak") && grepl("last 3", lab2, fixed = TRUE)) {
...
share$education2 <- datawizard::to_factor(sjlabelled::set_labels(share$education, labels = c("low", "mid", "high")))
...
share$both_waves <- sjlabelled::set_label(share$both_waves, label = "N of waves participated")
share$both_waves <- sjlabelled::set_labels(share$both_waves, labels = c("one", "both"))
...
s2 <- s2 |>
  ...
  dplyr::mutate(
    cac003_1 = if (isTRUE(any(cac003_1 == 1))) 1
...
share$covid_affected_ever <- recode_into(
  # never covid?
  (is.na(covid_symptoms_ever) | covid_symptoms_ever == 0) &
  (is.na(covid_positive_ever) | covid_positive_ever == 0) &
  (is.na(covid_hospital_ever) | covid_hospital_ever == 0) ~ 1,
  # ever positive, but no symptoms?
  covid_positive_ever == 1 &
  (is.na(covid_symptoms_ever) | covid_symptoms_ever == 0) &
  (is.na(covid_hospital_ever) | covid_hospital_ever == 0) ~ 2,
  # ever symptoms?
  covid_symptoms_ever == 1 &
  (is.na(covid_hospital_ever) | covid_hospital_ever == 0) ~ 3,
  # hpstitalized?
  covid_hospital_ever == 1 ~ 4,
  data = share,
  default = as.numeric(NA)
)
share$covid_affected_ever <- sjlabelled::set_label(share$covid_affected_ever, label = "Ever affected by Covid, and how severe")
share$covid_affected_ever <- sjlabelled::set_labels(share$covid_affected_ever, labels = c("never", "tested positive, no symptoms", "symptoms", "hospitalized"))
...
s8$covid_affected8 <- recode_into(
  (is.na(covid_symptoms8) | covid_symptoms8 == 0) & (is.na(covid_positive8) | covid_positive8 == 0) & (is.na(covid_hospital8) | covid_hospital8 == 0) ~ 1,
  covid_positive8 == 1 & (is.na(covid_symptoms8) | covid_symptoms8 == 0) & (is.na(covid_hospital8) | covid_hospital8 == 0) ~ 2,
  covid_symptoms8 == 1 & (is.na(covid_hospital8) | covid_hospital8 == 0) ~ 3,
  covid_hospital8 == 1 ~ 4,
  data = s8,
  default = as.numeric(NA)
)
s8$covid_affected8 <- sjlabelled::set_label(s8$covid_affected8, label = "Affected by Covid until mid 2020, and how severe")
s8$covid_affected8 <- sjlabelled::set_labels(s8$covid_affected8, labels = c("never", "tested positive, no symptoms", "symptoms", "hospitalized"))
...
s9$covid_affected9 <- recode_into(
  (is.na(covid_symptoms9) | covid_symptoms9 == 0) & (is.na(covid_positive9) | covid_positive9 == 0) & (is.na(covid_hospital9) | covid_hospital9 == 0) ~ 1,
  covid_positive9 == 1 & (is.na(covid_symptoms9) | covid_symptoms9 == 0) & (is.na(covid_hospital9) | covid_hospital9 == 0) ~ 2,
  covid_symptoms9 == 1 & (is.na(covid_hospital9) | covid_hospital9 == 0) ~ 3,
  covid_hospital9 == 1 ~ 4,
  data = s9,
  default = as.numeric(NA)
)
s9$covid_affected9 <- sjlabelled::set_label(s9$covid_affected9, label = "Affected by Covid until mid 2021, and how severe")
s9$covid_affected9 <- sjlabelled::set_labels(s9$covid_affected9, labels = c("never", "tested positive, no symptoms", "symptoms", "hospitalized"))
...
share$sphus2 <- recode_values(share$caph003_, recode = list(`0` = 3:5, `1` = 1:2))
share$sphus2 <- sjlabelled::set_labels(share$sphus2, labels = c("poor", "excellent"))

share$health_past3months <- recode_values(
  share$cah002_,
  recode = list(`0` = c(1, 3), `1` = 2)
)
share$health_past3months <- sjlabelled::set_labels(
