...
coronavirus <- coronavirus |>
  ...
  dplyr::filter(date_num <= Fieldwork, type == "confirmed") |>
  dplyr::ungroup()

total_cases <- coronavirus |>
  dplyr::group_by(country) |>
  dplyr::summarise(total_covid_cases = sum(cases))
...
total_cases <- dplyr::bind_rows(total_cases_wave8, total_cases_wave9)
colnames(total_cases)[1] <- "land"


share <- dplyr::left_join(share, total_cases, by = c("land", "wave"))
